...
static void initDeckSans(struct deck* const restrict deck, const struct player* const restrict player, const struct deck* const restrict pile)
{
	{	assert(deck);
		assert(player);
...
uint_fast32_t aiRandom(const struct aistate* const restrict as)
{
	...
	if(unlikely(plistGet(as->pl, MUPACK(ret))->n == ncards || ncards > CPP + ExtraDraw))
		return ret;

	p = stateToPlayer(as->gs);
	populateCIH(as->gs, cih);
	cur = evalPlayerf(p, as->gs->nplayers, cih[0]);
	...
	if(!as->gs->drew) {
		...
		while(td.n--) {
			tp = *p;
			tp.c[tp.n++] = *td.top++;
			hmb += (evalPlayerf(&tp, as->gs->nplayers, cih[0]) + ((float)DrawThreshold2/100.0) < cur);
		}
	/* We can't draw, don't even consider passing if there are only two players
	 * and they just played an 8 */
	} else {
		hmb += OFFS2 - (as->gs->nplayers != 2 || as->gs->eightSuit == Unknown) * EM / 1000.0;
		...
		if(likely(playsuit != passsuit || playval != passval || isMagicCard(playval))) {
			...
			while(td.n--) {
				...
				lval = getVal(c);
				if(unlikely(lval != 8))
					hmb += (lsuit == playsuit && lsuit != passsuit) * ((float)PlayS2/100.0)
						-  (lsuit == passsuit && lsuit != playsuit) * ((float)PassS2/100.0)
						+  (lval == playval && lval != passval) * ((float)PlayV2/100.0)
						-  (lval == passval && lval != playval) * ((float)PassV2/100.0);
			}
		}
	}
